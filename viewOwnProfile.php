...
elseif($_SESSION['loggedInUsername'])
{
    $user=$_SESSION['loggedInUsername'];
}
...
<div class="container" style="text-align: center ; background-color: mediumblue ">
    ...
    else
    {
        ...
        //print_r($_GET);
        //print_r($_POST);
        //$myUserName = $_GET["loggedInUsername"];
        //$myUserName = $_SESSION['loggedInUsername'];//session theke nite hobey
        $myUserName = $user;
        $my_sql_query = " SELECT *
                        FROM userinfo 
        ...
        if ($result->num_rows > 0)
        {
            while ($row = $result->fetch_assoc())
            {
                ...
                echo "<br>";
                echo "<div class=\"container-fluid\" style=\"background-color: black\">";
                $dynamic_location="updateOwnProfile.php?unSent="."$row[userName]";
                echo
                    "<br>".
